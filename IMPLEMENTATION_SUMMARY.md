# ArtWait implementation summary
...
ArtWait is a VS Code extension with a bundled Tauri companion. On activation,
...
`%LOCALAPPDATA%\ArtWait\app\artwait.exe`; hook configuration targets that
stable path so a VS Code extension update does not invalidate it.

## Current artifact

- `packages/vscode-extension/artwait-vscode-win32-x64-0.1.7.vsix`
- Windows x64 only
...
Version 0.1.4 is published as `artwait.artwait-vscode`; the locally built
